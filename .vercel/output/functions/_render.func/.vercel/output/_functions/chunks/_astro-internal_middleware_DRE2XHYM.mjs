...
import { A as AstroError, R as ResponseSentError } from './astro/assets-service_C5E55Y4e.mjs';

function onRequest$1 ({ locals, request, cookies, redirect }, next) {
    // interceptar los datos de una solicitud.
    // opcionalmente, modifica las propiedades en `locals`.
    const user = cookies.has('code_recovery');
    if(!user) console.log("no existe");
    if(user) console.log("si existe");
